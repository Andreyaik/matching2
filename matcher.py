...
def matcher(original, lookup, k_matches=5, ngram_length=3):
    ...
    - Then extract the `original`, `n-matches` the closest lookups, and calculate
    ...
    # Assertions
    assert all(
        [isinstance(x, str) for x in original]
    ), "Original contains non-str elements!"
    assert all(
        [isinstance(x, str) for x in lookup]
    ), "Lookup contains non-str elements!"
    assert isinstance(k_matches, int), "k_matches must be an integer"
    assert k_matches < len(
        lookup
    ), "k_matches must be shorter than the total length of the lookup list"
    assert isinstance(ngram_length, int), "ngram_length must be an integer"

    # Enforce list type, set to lower
    ...
    def ngrams_user(string, n=ngram_length):
        return ngrams(string, n, reg=r"[,-./]|\sBD")
    ...
    # Use NearestNeighbors model to obtain the nearest matches in lookup dataset. Vectorize first.
...
if __name__ == '__main__':
    df = matcher([pd.read_csv('data/new_items_test.csv').loc[0, 'name_1']],
                 list(pd.read_csv('data/dataset_test_100.csv')['name_2'].values),
                 k_matches=5, ngram_length=3)
    df.to_csv('data_out/after_tfidf.csv')
